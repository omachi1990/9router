...
export function logUsage(provider, usage, model = null, connectionId = null, apiKey = null) {
  ...
  saveRequestUsage({ model, provider, connectionId, tokens, apiKey: apiKey || undefined }).catch((e) => { console.error("SAVE USAGE ERROR:", e); });
  appendRequestLog({ model, provider, connectionId, tokens, status: "200 OK" }).catch((e) => { console.error("SAVE USAGE ERROR:", e); });
}
